// VERSION MARKER - if you open the browser console (F12) and DON'T see this exact line, the
    ...
    console.log("BIG HEIST PANEL BUILD: 2026-07-23-getaway-vehicle-art");
    ...
    document.addEventListener("visibilitychange", function () {
        if (document.visibilityState === "visible" && authToken) fetchMyData();
    });
    ...
    function renderPerpSheet(data) {
        ...
        if (shopButton) {
            shopButton.addEventListener("click", function () {
                // Instant, client-side - no queueAction/server round-trip needed just to browse,
                // since shopListing is passive data already sitting in the regular poll response.
                // Buttons are fully interactive immediately now - Purchase Script/Sell Item/
                // Finders Fee all re-check the heat/haggle ban directly at the moment of the
                // actual transaction, so there's no longer a window where a click could slip
                // through before a rejection lands. The background checkShopHeat roll below still
                // runs (for the "walked in, then got turned away" cinematic if it fails), it just
                // no longer needs to freeze anything while it's in flight.
                showShopBrowser = true;
                if (lastFetchedData) renderPerpSheet(lastFetchedData);
                // Fired alongside, not instead of, the instant open above - a background roll
                // against personal+show heat. Only ever overrides the shop view (via
                // panelOverride, a few seconds later once the panel polls again) if the roll
                // fails - the common low-heat case never notices this at all.
                queueAction("checkShopHeat", {});
            });
        }
